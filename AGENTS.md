...
- A node's body follows a git-commit shape (first non-empty line = subject/title, optional blank
  line, remainder = body) — see `docs/design/composable-redesign.md` (`### Fields`).
...
go test -tags integration ./tests/ # Run integration tests
go test -cover ./...               # Check coverage
```

**Key testing areas:**
- **Unit:** Parsers, writers, service logic (mock dependencies)
- **Integration:** Multi-module workflows (input → storage → retrieval)
- **TUI Visual:** xterm.js + gotty + Playwright MCP for UX validation
- **E2E:** "Day's work" validation (future - verify all connections work)
...
This includes:
- Detailed test pyramid explanation
- Unit test patterns and examples
- Integration test workflows
- TUI visual/UX testing with Playwright
- End-to-end "day's work" validation concept
- Running tests (all commands and options)
- Writing good tests (DOs and DON'Ts)
- Test data management (golden files, in-memory DB)
