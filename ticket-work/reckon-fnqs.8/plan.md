...
Confirmed zero external references: `wins_view.go`, `intention_list.go`, `task_picker.go`, `task_picker_test.go`, `task_picker_example_test.go`, and `schedule_view.go`.

- `wins_view.go`, `intention_list.go`, `task_picker.go` (+2 tests): not among the 4 panes, no caller. AC §2.1 explicitly blesses delete-or-decouple for these.
...
2. **No-arg keys** `t`/`x`/`i`/`c`: immediately `dispatchTodayAct(vaultDir, ref, key, "", noLog)`.
3. **Arg keys** `d`/`D`/`p`: open an input sub-flow, then `dispatchTodayAct(vaultDir, ref, key, arg, noLog)`:
...
| 6 | press `x` → `completeDurableTodoNode`; plain→`done`; `repeat:`→`scheduled` advances, `state:open` | `tui_test.go::TestAgendaActDone` + `TestAgendaActDoneRecurring` |
