...
func appendLogEntry(logDir, day, hhmm, author, body string) (logAddResult, error) {
	...
	return writeLogEntryBlock(logDir, day, hhmm, id, block)
}

// appendDidLogEntry is appendLogEntry's v1-T6 sibling: it writes the same
// create-vs-append day-file skeleton, but the entry block additionally
// carries a `did:: <ruleTarget>` marker (node.RenderLogEntryWithDid) so
// LogParser derives a Link{Rel:"did"} audit edge from the new entry to the
// completed recurring todo (plan.md "Exact `did::` Marker Syntax"). Used by
// doneRecurringTodo (todo.go) -- never by the plain `rk add` path, which
// keeps calling appendLogEntry unchanged.
//
// The embeddedHeaderRe guard is applied to author defensively even though
// body is machine-generated by doneRecurringTodo: author may still be
// user-supplied via --author.
func appendDidLogEntry(logDir, day, hhmm, author, body, didTarget string) (logAddResult, error) {
	if embeddedHeaderRe.MatchString(author) {
		return logAddResult{}, fmt.Errorf(`add: author must not contain a line starting with "## " (would be mis-split as a new entry)`)
	}
	if embeddedHeaderRe.MatchString(body) {
		return logAddResult{}, fmt.Errorf(`add: body must not contain a line starting with "## " (would be mis-split as a new entry)`)
	}
	id := node.Mint()
	block := node.RenderLogEntryWithDid(hhmm, author, id, didTarget, body)
	return writeLogEntryBlock(logDir, day, hhmm, id, block)
}

// writeLogEntryBlock is the shared create-or-append tail for appendLogEntry
// and appendDidLogEntry: create log/<day>.md via the NewNode -> Render ->
// Parse -> writeFileAtomic recipe if absent, else append block strictly at
// EOF. block is the exact, already-rendered entry bytes (either
// node.RenderLogEntry's or node.RenderLogEntryWithDid's output); id/hhmm are
// only needed to compose the returned logAddResult.
func writeLogEntryBlock(logDir, day, hhmm, id, block string) (logAddResult, error) {
