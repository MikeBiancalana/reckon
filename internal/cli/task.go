...
var taskListCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if taskFormatFlag != "" {
			...
			switch format {
			case FormatJSON:
				if err := formatTasksJSON(tasks); err != nil {
					return fmt.Errorf("failed to format tasks as JSON: %w", err)
				}
			case FormatTSV:
				if err := formatTasksTSV(tasks); err != nil {
					return fmt.Errorf("failed to format tasks as TSV: %w", err)
				}
			case FormatCSV:
				if err := formatTasksCSV(tasks); err != nil {
					return fmt.Errorf("failed to format tasks as CSV: %w", err)
				}
			}
			return nil
		}
  ...
  rk task delete --match auth`,
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if !quietFlag {
			...
			// In quiet mode, auto-confirm for scripting
			response = "y"
		}
