...
// migrateLegacyCmd runs the one-shot legacy-data importer: it reads gen-1
// task files, SQLite-backed notes/checklists, and legacy journal day files,
// and writes vault-native canonical node files that carry the old ids as
...
var migrateLegacyCmd = &cobra.Command{
	...
	SilenceUsage: true,
	Args:         cobra.NoArgs,
	RunE:         runMigrateLegacyE,
}

var (
	migrateLegacyDryRunFlag bool
	migrateLegacyVerifyFlag bool
	migrateLegacySourceFlag string
)

func init() {
	f := migrateLegacyCmd.Flags()
	f.BoolVar(&migrateLegacyDryRunFlag, "dry-run", false, "Preview the migration; write no files, touch no index/cache")
	f.BoolVar(&migrateLegacyVerifyFlag, "verify", false, "Verify a completed migration against a rebuilt index instead of migrating")
	f.StringVar(&migrateLegacySourceFlag, "source", "", "Legacy data root (default: the legacy data directory)")

	migrateCmd.AddCommand(migrateLegacyCmd)
}

// resetMigrateLegacyFlags restores migrate-legacy flag variables to their
// defaults and clears their pflag Changed state, mirroring the other v1
// commands' reset helpers.
func resetMigrateLegacyFlags(cmd *cobra.Command) {
	migrateLegacyDryRunFlag = false
	migrateLegacyVerifyFlag = false
	migrateLegacySourceFlag = ""
...
// migrateLegacyResult is the structured summary of one `rk migrate legacy`
// run, printed via internal/output once the importer itself is implemented.
type migrateLegacyResult struct {
	Tasks              migrateLegacyTypeResult `json:"tasks"`
	Notes              migrateLegacyTypeResult `json:"notes"`
	ChecklistTemplates migrateLegacyTypeResult `json:"checklist_templates"`
	ChecklistRuns      migrateLegacyTypeResult `json:"checklist_runs"`
	JournalDays        migrateLegacyTypeResult `json:"journal_days"`

	FoldedTaskNotes   int `json:"folded_task_notes,omitempty"`
	RewrittenTaskRefs int `json:"rewritten_task_refs,omitempty"`
}

type migrateLegacyTypeResult struct {
	Created []migrateLegacyRecordEntry  `json:"created"`
	Skipped []migrateLegacySkippedEntry `json:"skipped"`
	Errored []migrateLegacyErroredEntry `json:"errored"`
}
...
func (r migrateLegacyResult) Pretty() string {
...
type migrateLegacyRecordEntry struct {
	SourceID string `json:"source_id"`
	Path     string `json:"path"`
	ULID     string `json:"ulid"`
}

type migrateLegacySkippedEntry struct {
	SourceID string `json:"source_id"`
	Reason   string `json:"reason"`
}

type migrateLegacyErroredEntry struct {
	SourceID string `json:"source_id"`
	Error    string `json:"error"`
}

// migrateLegacyVerifyResult is the structured summary of one `rk migrate
// legacy --verify` run, printed via internal/output once the importer itself
// is implemented.
type migrateLegacyVerifyResult struct {
...
// unresolved alias.
func (r migrateLegacyVerifyResult) Pretty() string {
	var b strings.Builder
	status := "OK"
	if !r.OK {
		status = "FAILED"
...
func runMigrateLegacyE(cmd *cobra.Command, args []string) error {
	defer resetMigrateLegacyFlags(cmd)

	if migrateLegacyVerifyFlag && migrateLegacyDryRunFlag {
	...
	source := migrateLegacySourceFlag
	if source == "" {
	...
	imp := &textmigrate.Importer{Source: source, Dest: cfg.VaultDir, DryRun: migrateLegacyDryRunFlag}

	if migrateLegacyVerifyFlag {
		vr, err := imp.Verify()
		if err != nil {
			return fmt.Errorf("import: verify: %w", err)
		}
		res := toMigrateLegacyVerifyResult(vr)
	...
	res := toMigrateLegacyResult(report)
...
// toMigrateLegacyResult maps a textmigrate.Report onto the CLI's JSON-tagged
// result shape.
func toMigrateLegacyResult(r *textmigrate.Report) migrateLegacyResult {
	return migrateLegacyResult{
		Tasks:              toMigrateLegacyTypeResult(r.Tasks),
		Notes:              toMigrateLegacyTypeResult(r.Notes),
		ChecklistTemplates: toMigrateLegacyTypeResult(r.ChecklistTemplates),
		ChecklistRuns:      toMigrateLegacyTypeResult(r.ChecklistRuns),
		JournalDays:        toMigrateLegacyTypeResult(r.JournalDays),
		FoldedTaskNotes:    r.FoldedTaskNotes,
		RewrittenTaskRefs:  r.RewrittenTaskRefs,
	}
}

func toMigrateLegacyTypeResult(tr textmigrate.TypeResult) migrateLegacyTypeResult {
	created := make([]migrateLegacyRecordEntry, 0, len(tr.Created))
	for _, c := range tr.Created {
		created = append(created, migrateLegacyRecordEntry{SourceID: c.SourceID, Path: c.Path, ULID: c.ULID})
	}
	skipped := make([]migrateLegacySkippedEntry, 0, len(tr.Skipped))
	for _, s := range tr.Skipped {
		skipped = append(skipped, migrateLegacySkippedEntry{SourceID: s.SourceID, Reason: s.Reason})
	}
	errored := make([]migrateLegacyErroredEntry, 0, len(tr.Errored))
	for _, e := range tr.Errored {
		errored = append(errored, migrateLegacyErroredEntry{SourceID: e.SourceID, Error: e.Error})
	}
	return migrateLegacyTypeResult{Created: created, Skipped: skipped, Errored: errored}
}

// toMigrateLegacyVerifyResult maps a textmigrate.VerifyResult onto the CLI's
...
func toMigrateLegacyVerifyResult(vr *textmigrate.VerifyResult) migrateLegacyVerifyResult {
	...
	return migrateLegacyVerifyResult{
		Counts:             vr.Counts,
