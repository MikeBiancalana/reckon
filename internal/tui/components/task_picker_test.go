...
func TestTaskPicker_GetSelectedTaskID(t *testing.T) {
	...
	// After selection
	selectedTask := &IndexRow{ID: "task-1", Title: "Test task"}
...
func TestTaskPicker_FilterValue(t *testing.T) {
	row := IndexRow{ID: "task-1", Title: "Write comprehensive tests"}
	item := taskPickerItem{row: row}
...
func TestTaskPicker_ItemDescription(t *testing.T) {
	// Row with scheduled + deadline props
	scheduled := "2026-01-10"
	deadline := "2026-01-15"
	row1 := IndexRow{
		ID:    "task-1",
		Title: "Write tests",
		Props: map[string]string{"scheduled": scheduled, "deadline": deadline},
	}
	item1 := taskPickerItem{row: row1}
